...
def build_training_selector(args):
    return _training_selector(args)

def build_testing_selector(data_distribution=None, client_info=None, model_size=None):
    return _testing_selector(data_distribution, client_info, model_size)

class _testing_selector:
    """HeaPS's testing selector
...
def get_rank(inputlist):
    ranklist = (np2.argsort(-np2.array(inputlist))+1) / len(inputlist)
    return ranklist

class _training_selector(object):
    """HeaPS's training selector
    """
    def __init__(self, args, sample_seed=233):
        ...
        self.commuTimeofClients={}
        self.compuTimeofClients={}
        self.RankComMu={}
        self.RankComPu={}

        client_path2 = '/home/zjlab/yangduo/HeaPS/device_info/client_device_capacity_client_3500'
        with open(client_path2, 'rb') as fin:
            self.ClientsCommunication =  pickle.load(fin)

        self.sortedClients = []
        self.available_member = []
        self.best_leader = {}

    def generateRankList(self, commuTimeClients=None, compuTimeClients=None):
        self.commuTimeofClients = commuTimeClients
        self.compuTimeofClients = compuTimeClients
        Listcommunication=[]
        Listcomputation=[]
        for client in commuTimeClients.keys():
            Listcommunication.append(commuTimeClients[client])
            Listcomputation.append(compuTimeClients[client])
        
        rankcomMu = (np2.argsort(-np2.array(Listcommunication))+1) / len(Listcommunication)
        rankcomPu = (np2.argsort(-np2.array(Listcomputation))+1) / len(Listcomputation)
        index = 0
        for client in commuTimeClients.keys():
            self.RankComMu[client] = rankcomMu[index]
            self.RankComPu[client] = rankcomPu[index]
            index += 1
    ...
    def getTopK(self, numOfSamples, cur_time, feasible_clients):
        ...
        try: 
            ...
            self.sortedClients = sortedClientUtil
            logging.info("scores are: {},\n sorted keys: {}".format(scores,sortedClientUtil))
            logging.info("feasible_clients:{}, clients_with_score:{}".format(len(feasible_clients), len(sortedClientUtil)))
            # take cut-off utility
            cut_off_util = scores[sortedClientUtil[exploitLen]] * self.args.cut_off_util

            pickedClients = []
            pickedexploitClients = []
            pickedTotalClients = []
            ...
            logging.info("from exploitset {} exploit {}".format(len(pickedClients), exploitLen))
            pickedexploitClients = list(np2.random.choice(pickedClients, exploitLen, p=poss_sample, replace=False))
            self.exploitClients = pickedexploitClients
            logging.info("exploitClients: {}".format(self.exploitClients))

            # exploration
            _unexplored = [x for x in list(self.unexplored) if int(x) in feasible_clients]
            logging.info("unexplored:{}, common_part_w_feasible:{}".format(len(self.unexplored), len(_unexplored)))
            ...
            if len(_unexplored) > 0:
                ...
                for cl in _unexplored:
                    ...
                    init_reward[cl] *= self.RankComMu[key]
                ...
                exploreLen = min(len(_unexplored), numOfSamples - len(pickedexploitClients))
                ...
                poss_sample=np2.asarray(poss_sample).astype('float64')
                poss_sample = poss_sample / np2.sum(poss_sample)
                logging.info("exploration poss sum is {}".format(np2.sum(poss_sample)))
                ...
                logging.info("from exploreset {} explore {}".format(len(pickedUnexploredClients), exploreLen))
                ...
                logging.info("exploreClients: {}".format(self.exploreClients))
            else:
                # no clients left for exploration
                self.exploration_min = 0.
                # self.exploration = 0.
            pickedTotalClients = pickedexploitClients + pickedUnexplored
            logging.info("pickedTotalClients: {}".format(len(pickedTotalClients)))

            while len(pickedTotalClients) < numOfSamples:
                nextId = self.rng.choice(orderedKeys)
                if nextId not in pickedTotalClients:
                    pickedTotalClients.append(nextId)

            top_k_score = []
            for i in range(min(3, len(pickedTotalClients))):
                clientId = pickedTotalClients[i]
            ...
            logging.info("At round {}, UCB exploited {}, augment_factor {}, exploitLen {}, exploreLen {}, un-explored {}, exploration {}, round_threshold {}, sampled score is {}"
                .format(cur_time, numOfExploited, augment_factor/max(1e-4, exploitLen), exploitLen, exploreLen, len(self.unexplored), self.exploration, self.round_threshold, top_k_score))
        ...
        return pickedTotalClients

    def get_sortedClients(self):
        return self.sortedClients
    ...
    def resample_Members(self, Duration, leadersToRun, cur_time, feasible_clients):
        viable_clients = feasible_clients if feasible_clients is not None else set([x for x in self.totalArms.keys() if self.totalArms[x]['status']])
        select_members = []
        MemberClients = []
        self.available_member = list(set(self.sortedClients)-set(leadersToRun))
        # self.available_member = PendingClients
        memberutil = {}
        bestleaderTime = {}
        self.best_leader = {}
        for key in self.available_member:
            # member client key choose the leader client i which minimize the total communication time from key to i then to the server, max the iterations
            member_leader_util = {}
            member_choose_leader_time = {}
            for i in leadersToRun:
                member_leader_util[i] = max(0, Duration - self.args.model_size * self.args.clock_factor / float(self.ClientsCommunication[key][i]) - self.commuTimeofClients[i]) / self.compuTimeofClients[key] + 1
                member_choose_leader_time[i] = self.args.model_size * self.args.clock_factor / float(self.ClientsCommunication[key][i]) + self.commuTimeofClients[i]
            
            memberutil[key] = max(list(member_leader_util.values()))
            bestleaderTime[key] = min(list(member_choose_leader_time.values()))
            self.best_leader[key] = min(member_choose_leader_time,key=member_choose_leader_time.get)

        logging.info("memberutil:{}".format(memberutil))
            
        # rankcommuclients = get_rank(list(Listcommumember))
        # rankcommu_clients = {}
        # index_rcc = 0
        # for key in Listcommumember:
        #     rankcommu_clients[key] = rankcommuclients[index_rcc]
        #     index_rcc += 1

        moving_reward2, staleness2, allloss2 = [], [], {}
        for clientId in self.available_member:
            if self.totalArms[clientId]['reward'] > 0:
                creward2 = self.totalArms[clientId]['reward']
                moving_reward2.append(creward2)
        _, min_reward2, range_reward2, _, clip_value = self.get_norm(moving_reward2, 0.98)
        scores_member = {}
        for key in self.available_member:
            creward2 = self.totalArms[key]['reward']
            sc_member = ((creward2 - min_reward2)/float(range_reward2) \
                + math.sqrt(0.1*math.log(cur_time)/self.totalArms[key]['time_stamp'])) #* self.round_decay_factor
            sc_member *= memberutil[key] #rankcommu_clients[key] * self.rankcomputation[key]
            scores_member[key] = sc_member

        memberLen = min(len(self.available_member), int(self.total_worker* self.args.overcommit*(1-self.args.leaderFactor)))
        poss_member=[scores_member[key] for key in self.available_member]
        poss_member=np2.asarray(poss_member).astype('float64') 
        poss_member=poss_member / np2.sum(poss_member)
        select_members = list(np2.random.choice(self.available_member, memberLen, p=poss_member, replace=False))
        for mm in select_members:
            if mm in viable_clients and bestleaderTime[mm] < self.commuTimeofClients[mm]:
                MemberClients.append(mm)
        logging.info("available_member:{}, memberLen:{}".format(len(self.available_member), len(MemberClients)))
        logging.info("MemberClients: {}".format(MemberClients))

        return MemberClients
    
    def getMigrationMatrix(self, Leader_Clients, Member_Clients):
        MigrationMatrix = {}
        CommuTimeMatrix = {}

        # MigrationMatrix = [[0 for column in range(len(Member_Clients))] for row in range(len(Leader_Clients))]
        # index_leader = 0
        # index_member = 0
        for i in Leader_Clients:
            if i not in MigrationMatrix:
                MigrationMatrix[i] = []
            if i not in CommuTimeMatrix:
                CommuTimeMatrix[i] = []
            for j in Member_Clients:
                if self.best_leader[j] == i:
                    # MigrationMatrix[index_leader][index_member] = 1
                    # index_member += 1 
                    # index_leader = 0
                    MigrationMatrix[i].append(j)
                    CommuTimeMatrix[i].append(self.args.model_size* self.args.clock_factor / float(self.ClientsCommunication[i][j]))
                    # break
                # index_leader += 1
        # logging.info("Member to Leader Path: ".format(MigrationMatrix))
        return MigrationMatrix, CommuTimeMatrix
