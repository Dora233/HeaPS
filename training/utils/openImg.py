...
class OPENIMG():
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, imgview=False,num_classes=596):
        ...
        with open(os.path.join(self.processed_folder, 'classTags'), 'r', encoding='utf-8') as fin:#, errors = 'ignore'
            self.classes = [tag.strip() for tag in fin.readlines()]
            # logging.info("len(self.classes): {}".format(len(self.classes))) # 601

        # logging.info("num_classes: {}".format(num_classes)) # 60
        self.classMapping = {k.split(',')[0]:v for (k,v) in self.class_to_idx.items() if v in range(num_classes)}
