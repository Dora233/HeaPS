...
def process_cmd(yaml_file):
    ...
    ps_cmd = f" {yaml_conf['python_path']}/python {yaml_conf['exp_path']}/param_server.py {conf_script} --this_rank=0 --learners={learner_conf} --gpu_device=0"
    ...
    for worker, gpu in zip(worker_ips, total_gpus):
        ...
        for gpu_device  in range(len(gpu)):
            for _  in range(gpu[gpu_device]):
                worker_cmd = f" {yaml_conf['python_path']}/python {yaml_conf['exp_path']}/learner.py {conf_script} --this_rank={rank_id} --learners={learner_conf} --gpu_device={gpu_device}"
                rank_id += 1
